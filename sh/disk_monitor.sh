...
function start(){
	...
	if [ $fire_alert == 0 ]; then
		echo "${dttimes} ${log_info} all disks temperature stay low, warning line ${ALERT_LEVEL}" >> $log_file
	else
		echo "${dttimes} ${log_warn} fire temperature alert, plz check wx message, warning line ${ALERT_LEVEL}" >> $log_file
	fi
}
