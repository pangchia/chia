...
final_plot_size=102
# 1~100 unit: %
remote_disk_used_percent_threshold=85
	...
	if [ ! -n "${copyPlotProcess}" ] ; then
		...
		if [  -n "$last_file_name" ]; then
			...
			if [ $plot_not_exist == 1 ]; then
				...
				remote_query_result=`ssh root@${remote_hvst_ip} "df -BG|grep '${remote_hvst_dir}' "`
				...
				remote_disk_used_perc=`echo ${remote_query_result} | awk -F' ' '{printf $5"%"}' |  cut -d "%" -f1`
				...
				echo "remote_disk_used_perc : ${remote_disk_used_perc}%"  >> $trucking_log_file

				if [ $remote_disk_used_perc -gt $remote_disk_used_percent_threshold ]; then

					echo "${dttimes} ${log_warn} remote disk <${remote_hvst_ip}:${remote_hvst_dir}> used: ${remote_disk_used_perc}% " >> $trucking_log_file
					message="ip: ${remote_hvst_ip} \ndisk: ${remote_hvst_dir} \nused: ${remote_disk_used_perc}%"
					...
					echo "${dttimes} wx push callback :${callback}"
				fi
